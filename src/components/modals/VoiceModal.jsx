  ...
  async function finish() {
    ...
    try {
      ...
      const out = await store.api('/ai/voice', {
        method: 'POST',
        body: JSON.stringify({ audio: b64, mimeType: rec.mimeType, history: store.noteHistory().slice(0, 60), customCategories: store.customCategories.map((c) => c.name), holdings: store.holdings.map((h) => h.name) }),
      });
