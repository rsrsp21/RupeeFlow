...
export async function POST(request) {
  try {
    ...
    const { text, history, customCategories, holdings, accounts, groups, today } = await request.json().catch(() => ({}));
    if (!text) throw new HttpError('Text required');
    return jsonRes(await parseText(text, history || [], customCategories || [], holdings || [], accounts || [], groups || [], today || ''));
