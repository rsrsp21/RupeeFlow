...
export async function POST(request) {
  try {
    ...
    const { audio, mimeType, history, customCategories, holdings, accounts, groups, today } = await request.json().catch(() => ({}));
    if (!audio) throw new HttpError('Audio required');
    return jsonRes(await parseVoice(audio, mimeType, history || [], customCategories || [], holdings || [], accounts || [], groups || [], today || ''));
