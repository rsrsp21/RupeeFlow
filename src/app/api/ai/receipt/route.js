...
export async function POST(request) {
  try {
    ...
    const { image, mimeType, history, customCategories, accounts } = await request.json().catch(() => ({}));
    if (!image) throw new HttpError('Image required');
    return jsonRes(await parseReceipt(image, mimeType, history || [], customCategories || [], accounts || []));
